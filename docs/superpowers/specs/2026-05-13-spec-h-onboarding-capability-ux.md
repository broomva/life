# Spec H — Life Onboarding & Capability UX

**Date**: 2026-05-13
**Status**: Draft (Phase H-Sub-A in progress via [#1242](https://github.com/broomva/life/pull/1242); H-Sub-B…O queued)
...
**Linear**: umbrella issue (Urgent, 0 points) with 15 sub-phase children H-Sub-A…O (priorities + estimates inline); filed against the **Life** project on the broomva team after user approval of this spec.
...
The deep-research analysis of Zero (zero.xyz) and Coinbase Agentic.market that produced this spec [^1] surfaced a second-order observation: the substrate side of Life is *ahead* of every named competitor — Anima's six-backend custody is genuinely deeper than Zero's flat private key. The first-run UX side is *behind* — Zero compresses to one command what Life today spreads across three. Spec H closes that gap and then extends past it.

This is Spec H: the developer-facing surface that turns Life's substrate advantages into a one-command onboarding story and a Life-native capability discovery layer.
...
### L6-H3 — InProcess is the default custody backend, but never the only build target
...
### L6-H4 — Capability discovery is event-sourced via Lago, not state-stored
...
### L6-H5 — Reviews are deferred-write events paired to payment events
...
### L6-H6 — `PaidCapability` is a sibling of `Tool`, not a wrapper
...
### Capability substrate (Phase H.C)
...
```rust
// crates/aios/aios-protocol/src/capability.rs (NEW — H-Sub-J)
...
The registry projection folds the event stream and exposes `search(query) -> Vec<CapabilityListing>` via BM25 + graph walk. Distribution happens by either centralising at one Lago (single-host) or fanning out via Spaces gossip (H-Sub-K — agents publish advertisements to a shared `#capabilities` channel; other agents fold the channel into their local projection).

### Operator surface (Phase H.D)
...
### H-Sub-A — Anima identity bootstrap on `life init` ✅ SHIPPED
...
**Dependencies**: H-Sub-A.

**Estimate**: 2 points. **Priority**: High (unblocks H-Sub-C and H-Sub-D).
...
**Dependencies**: H-Sub-A, H-Sub-B (for the event record). Server side can ship in parallel.

**Estimate**: 3 points. **Priority**: Medium. Server-side adds 2 more for the merchant + dedupe DB.

### H-Sub-E — `--custody=` flag on `life init`

**Scope**: Add `--custody=in_process|vault|tpm|webcrypto|hardware|soma` to `life init`. Each backend has its own config requirements (Vault URL, TPM PKCS#11 module path, etc.) gathered interactively or via additional flags. Builds gated by Cargo features per L6-H3.
...
- All Spec D backends supported by Phase H.B end-state

**Dependencies**: H-Sub-A. (Other backends are already shipped in Spec D D-Sub-B…F.)
...
**Dependencies**: H-Sub-E (for the secret).

**Estimate**: 1 point.

### H-Sub-H — `capability-registry` crate

**Scope**: New `crates/capability-registry/` (sibling of `lago`). Maintains a Lago projection of `capability.advertised`, `capability.reviewed`, and `capability.deprecated` events. Exposes `pub fn search(query: &str, opts: SearchOpts) -> Vec<CapabilityListing>` over a BM25 index built from the projection. Reads from the local project Lago by default; optionally federates with Spaces channels via H-Sub-K.
...
**Dependencies**: H-Sub-B (Lago journal exists), H-Sub-J (`PaidCapability` trait — for the listing shape).
...
**Dependencies**: H-Sub-B.

**Estimate**: 3 points.

### H-Sub-J — `PaidCapability` trait in `aios-protocol`

**Scope**: Define a typed Rust trait for paid HTTP capabilities alongside the existing `Tool` trait. Implement adapter from `haima-x402` so any Bazaar-discovered or advertiser-supplied capability is a `PaidCapability`. Bridge in Praxis so paid tools and free tools share a single call shape. Implements L6-H6.
...
**Dependencies**: none — independent of H-Sub-H (but H-Sub-H consumes this).

**Estimate**: 3 points.

### H-Sub-K — Spaces gossip channel for capabilities

**Scope**: Opt-in Spaces channel `#capabilities` where agents publish their `capability.advertised` events. Subscribers fold the channel into their local registry projection. Implements decentralized discovery per L6-H4.
...
**Dependencies**: H-Sub-H, H-Sub-J. Requires `spaces/` to be running.
...
**Scope**: On explicit `life init --install-skills` or `life setup` prompt, write a `life` skill (a SKILL.md frontmatter file) to the relevant agent runtime directories so Claude Code, Cursor, Codex, and Windsurf can pick up the `life` command surface without manual configuration. Implements L6-H8.
...
**Dependencies**: H-Sub-A, H-Sub-E. Touches Spec D's lineage event shape — re-use rather than fork.
...
|---|---|---|---|---|---|
| **H.A** Bootstrap | H-Sub-A (✅), B, C, D | One-command-runnable agent | Q2 2026 (in progress) | H.B, H.C, H.D | Spec D D-Sub-A (✅) |
| **H.B** Custody | H-Sub-E, F, G | Production-grade key handling | Q3 2026 | H.D (skill install reads context) | H.A |
| **H.C** Capability | H-Sub-H, I, J, K | Discovery + reputation substrate | Q3-Q4 2026 | downstream apps that need cross-substrate discovery | H.A, H.B |
| **H.D** Operator UX | H-Sub-L, M, N, O | CLI polish + multi-agent | Continuous from Q3 2026 | — | H.B (for G context detection), H.A (for identity surface) |

Concretely, H-Sub-A ships in PR #1242. H-Sub-B and H-Sub-C are the next logical follow-ups (small, well-bounded). H-Sub-D, H-Sub-E, H-Sub-F can land in parallel — different files, no shared state. H-Sub-G depends on H-Sub-E (the secret-store path).

Phase H.C is the largest single block of work (H-Sub-H alone is 8 points). It can begin as soon as H-Sub-B lands (Lago journal in place), and H-Sub-J can run in parallel with H-Sub-H since they're separate trait surfaces. H-Sub-I (review ledger) can run independently. H-Sub-K (Spaces gossip) requires H-Sub-H + H-Sub-J.

Phase H.D is hygiene polish; safe to ship continuously alongside H.B and H.C. H-Sub-L is the smallest (1 point), H-Sub-N is the trickiest only because it touches multiple agent runtime layouts.

Total estimate: **45 points** across 15 sub-phases. At the current Life velocity (Spec D shipped 6 sub-phases × ~3-5 points each in 5 days; Spec C M5 shipped 5 sub-phases over ~2 weeks), Spec H is roughly **5-7 weeks of focused engineering** with two engineers, **8-10 weeks** with one.
...
```
H-Sub-A ✅ → H-Sub-B → H-Sub-C → H-Sub-J → H-Sub-H → H-Sub-I
        ↓
        H-Sub-E (for production deployments)
```

This unblocks:
1. A user can `life init` and have a working agent with identity + wallet + journal (H.A complete)
2. A user can `life init --custody=vault` and deploy to production safely (H-Sub-E)
3. The agent can discover both free Praxis tools and paid Haima capabilities through one trait (H-Sub-J)
4. There's a registry to search over (H-Sub-H)
5. Reviews accumulate to feed ranking (H-Sub-I)

H-Sub-D (welcome credit), H-Sub-K (Spaces gossip), H-Sub-N (skill installation), and H-Sub-O (lineage wallets) are all valuable but not on the critical path for "Life is usable end-to-end."

## Open Questions

1. **Welcome merchant operator**: Phase H-Sub-D's merchant runs *somewhere*. Broomva.tech is the natural host, but the merchant needs a dedicated wallet, a server-side double-claim DB, and gas budget. Worth a dedicated micro-spec before H-Sub-D ships.

2. **Spaces channel governance for H-Sub-K**: Who can create the `#capabilities` channel? Who can moderate? Who pays for the SpacetimeDB hosting? The federated discovery story requires a trust anchor that doesn't exist yet — maybe the broomva.tech-operated channel is the default, with self-hosted alternatives.

3. **`PaidCapability` price-discovery before invocation**: Phase H-Sub-J's trait surfaces price *at* invocation (via 402). For agents that need to budget across multiple capabilities, a pre-invocation `quote()` method would help. Worth adding as a v2 trait method, gated behind a feature flag.

4. **Skill content versioning for H-Sub-N**: When `life-cli` updates, the canonical skill file may need to change. Should re-installing detect a version mismatch? How does this interact with user-customized skills? Probably needs a `skill.toml` lockfile alongside.

5. **Trust tier escalation events**: H-Sub-M's `life trust show` reads the current tier, but how does a tier escalation actually happen? Via `anima.identity_attested` events from trusted issuers? Via a Haima payment threshold reached? This is a Spec G topic but worth flagging.

6. **Lineage budget enforcement vs Spec C lifegw rate limits**: H-Sub-O parent-set spend caps are an anima-side concept; lifegw also has rate limits. Are these the same primitive or composed? Probably composed — anima checks intent, lifegw enforces network-side — but worth a clear write-up.

## References

- `crates/cli/life-cli/src/init.rs` — current `life init` (extended by H-Sub-A in PR #1242)
- `crates/cli/life-cli/src/setup.rs` — interactive wizard (interacts with H-Sub-N skill install)
- `crates/anima/anima-identity/src/in_process.rs` — Spec D D-Sub-A backend used by H-Sub-A
- `crates/anima/anima-identity/src/{vault,tpm,webcrypto,hardware_wallet,soma,remote}.rs` — Spec D D-Sub-B…F backends consumed by H-Sub-E
- `crates/anima/anima-identity/src/seed.rs::EncryptedSeed` — used by H-Sub-F
- `crates/anima/anima-core/src/soul.rs::SoulBuilder` — used by H-Sub-A and extended by H-Sub-O
- `crates/anima/anima-lago/src/genesis.rs::create_genesis_event` — H-Sub-B genesis event source
- `crates/haima/haima-x402/src/bazaar.rs` — current `agentic.market` Bazaar client (parallel approach to H-Sub-H)
- `crates/haima/haima-wallet/src/backend.rs::WalletBackend` — used by H-Sub-D welcome flow
- `crates/lago/lago-knowledge/` — BM25 substrate reused by H-Sub-H
- `crates/spaces/life-spaces/` — networking substrate reused by H-Sub-K
