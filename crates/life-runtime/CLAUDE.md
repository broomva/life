...
| `arcan-proxy` | Typed tonic client for the arcan substrate. `ArcanCall` trait + `ArcanProxy` builder + retry policy + Tier-3 token attachment hook. |
| `lago-proxy` | Same shape, for lago. `LagoCall` trait + `LagoProxy` + retry policy. |
| `haima-proxy` | Same shape, for haima. `HaimaCall` trait + `HaimaProxy` + retry policy. Wallet types live here. |
| `anima-proxy` | Same shape, for anima. `AnimaCall` trait + `AnimaProxy` + retry policy. Account/Profile types live here. |
...
- **M5 sub-phase D** — Connection pools + circuit breakers + backpressure + observability + per-substrate `with_token` wiring + `JwksKey` PEM publish: pending.
