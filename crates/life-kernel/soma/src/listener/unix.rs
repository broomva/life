...
pub(crate) async fn serve(
    ...
    let listener = UnixListener::bind(&cfg.unix_socket)
        .map_err(|e| SomaError::Server(format!("bind {}: {e}", cfg.unix_socket.display())))?;

    if let Some(mode) = cfg.unix_socket_mode {
        std::fs::set_permissions(&cfg.unix_socket, std::fs::Permissions::from_mode(mode))
            .map_err(|e| SomaError::Server(format!("chmod socket: {e}")))?;
    }
    ...
    router
        ...
        .await
        .map_err(|e| SomaError::Server(format!("serve: {e}")))
}
...
fn prepare_socket_path(path: &Path) -> SomaResult<()> {
    ...
    if path.exists() {
        std::fs::remove_file(path)
            .map_err(|e| SomaError::Server(format!("unlink stale {}: {e}", path.display())))?;
    }
    Ok(())
}
...
fn chown_socket_group(path: &Path, group: &str) -> SomaResult<()> {
    tracing::warn!(
        target: "soma::listener::unix",
        group = %group,
        path = %path.display(),
